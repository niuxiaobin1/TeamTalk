...
public class GroupManagerAdapter extends BaseAdapter {
            ...
            public void onSuccess(Object response) {
                try {
                    ...
                    ((Activity) context).runOnUiThread(new Runnable() {
                        @Override
                        public void run() {
                            setGroupData(groupEntity, imChangeFriendRemarkRsp.getUserListList());
                        }
                    });
    ...
    private void setGroupData(GroupEntity entity, List<IMBaseDefine.UserInfo> userInfos) {
        int loginId = imService.getLoginManager().getLoginId();
        int ownerId = entity.getCreatorId();
        for (IMBaseDefine.UserInfo info : userInfos) {
            UserEntity user = ProtoBuf2JavaBean.getUserEntity(info);
            if (user != null) {
    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (position >= 0 && memberList.size() > position) {
            ...
            setHolder(holder, position, null, R.mipmap.add_group_member, "", null);
            holder.imageView.setOnClickListener(new View.OnClickListener() {
                @Override
            ...
            logger.d("groupmgr#onAddMsg - button");
            setHolder(holder, position, null, R.mipmap.del_group_member, "", null);
    ...
    private void setHolder(final GroupHolder holder, int position,
        ...
        if (null != holder) {
            ...
            if (avatarUrl != null) {
                ...
                holder.imageView.setImageResource(R.mipmap.default_user_icon);

...
import com.mogujie.tt.utils.menu.action.PopMenuAdapter;
...
public class Menu {
    ...
    private OnOpenZXingScanListener onOpenZXingScanListener;
        ...
        PopActionClickListener popActionClickListener = new PopActionClickListener() {
            ...
            public void onActionClick(int position, Object data) {
                ...
                if (TextUtils.equals(action.getActionName(), mActivity.getResources().getString(R.string.add_friend))) {
                    Intent intent = new Intent(mActivity, AddMoreActivity.class);
                    mActivity.startActivity(intent);
                }

                if (TextUtils.equals(action.getActionName(), mActivity.getResources().getString(R.string.add_scan))) {
                    if (onOpenZXingScanListener!=null){
                        onOpenZXingScanListener.onOpenZXing();
                    }
                }
...
//                    mActivity.startActivity(intent);
        ...
        if (menuType == MENU_TYPE_CONVERSATION) {
            ...
            action.setIconResId(R.mipmap.ic_nf);
        ...
        if (menuType == MENU_TYPE_CONTACT) {
            ...
            action.setIconResId(R.mipmap.ic_gc);
            action.setActionClickListener(popActionClickListener);
            menuActions.add(action);

            action = new PopMenuAction();
            action.setActionName(mActivity.getResources().getString(R.string.add_scan));
            action.setIconResId(R.mipmap.ic_scan);
            action.setActionClickListener(popActionClickListener);
            menuActions.add(action);
        }
    ...
    public void show() {
        ...
        mMenuWindow.setWidth((int)(ScreenUtil.getScreenWidth(mActivity)*0.39));
        mMenuWindow.setHeight(ViewGroup.LayoutParams.WRAP_CONTENT);
        mMenuWindow.setBackgroundDrawable(mActivity.getResources().getDrawable(R.mipmap.popup_bg));
        ...
        // 相对于 + 号正下面，同时可以设置偏移量
        mMenuWindow.showAtLocation(mAttachView, Gravity.RIGHT | Gravity.TOP, ScreenUtil.getPxByDp(15,mActivity), ScreenUtil.getPxByDp(80,mActivity));
        // 设置pop关闭监听，用于改变背景透明度
    ...
    public interface OnOpenZXingScanListener{
        void onOpenZXing();
    }

    public void addOnOpenZXingScanListener(OnOpenZXingScanListener onOpenZXingScanListener){
        this.onOpenZXingScanListener=onOpenZXingScanListener;
    }
}
